...
# 决定dataroot的值的操作，在创建数据集的参数时进行，所以在之后更改location的值不会改变dataroot
    ...
    opt = trainoption.parse("inpaint","cityscape",is_print=False)   # get training options

    opt.message="最基础的方法，只有风格"
    ...
    # opt.lambda_D2=1.0                                        
    opt.lambda_D_sim= 0.0                                       
    # opt.lambda_G2=2.0                                                                                             
    opt.lambda_mmaxL1=0.05                            
    opt.lambda_vgg=10.0           
    opt.lambda_vgggram=250.0    
    opt.lambda_L1=10
    
    opt.netD = "patchgan_multi"
    opt.netG = "adainunetgenerator"
    opt.location=1
    opt.niter = 80
    opt.niter_decay=40
    opt.name = "Base"
