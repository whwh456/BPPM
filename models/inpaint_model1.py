...
class InpaintModel(BaseModel):
    ...
    def modify_commandline_options(parser):
        ...
        parser.add_argument('--lambda_mmaxL1', type=int, default=0.1, help='让max和realB的L1距离变大')
    ...
    def selfgenerate(self):
        with torch.no_grad():
            if "withspade" in self.opt.netG:
                ...
                fake_B = self.netG(self.real_A, self.adainrefer, self.alpha)
            else:
                fake_B = self.netG(self.real_A)
        return fake_B
    ...
    def backward_G(self):
        ...
        if self.opt.lambda_L1>0.0:
            self.L1_loss=self.criterionL1(self.fake_B,self.real_B)
        else:
            self.L1_loss=0.0
        ...
        loss_G=self.G_GAN_loss+self.GAN_Feat_loss+self.VGG_loss+self.L1_loss+self.VGG_GRAM_loss+self.Difcanny_loss+self.G_GAN2_loss+self.ADAIN_loss
        loss_G.backward()
