...
# 决定dataroot的值的操作，在创建数据集的参数时进行，所以在之后更改location的值不会改变dataroot
    ...
    opt = trainoption.parse("edge","tokyo247",is_print=False)   # get training options

    opt.message="测试edge,有divloss模块,alpha为sum比例，lambda=0.001，有L1损失,原输入为onehot_mask_littleedge，目标输出为mask_edge"
    ...
    opt.location=1
    opt.niter = 20
    opt.name = "tokyo_edge_div_a6"
    opt.G_norm="spectralinstance"
    ...
    opt.input_nc = 3
    opt.output_nc = 1
    opt.resource_type = "onehot_mask_littleedge"
    opt.target_type = "full_edge"
