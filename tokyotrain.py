...
# 决定dataroot的值的操作，在创建数据集的参数时进行，所以在之后更改location的值不会改变dataroot
    ...
    opt.message="tokyo数据集上的imgu1_l1_3,lambda_mmaxL1=0.001"
    #opt.conditional_D=False
    # opt.lambda_difcanny=1.0
    opt.D_output_nc=1
    opt.lambda_D_sim = 1.0
    opt.return_mask=True
    opt.return_seg=True
    opt.lambda_vgg=10.0
    opt.lambda_vgggram=250.0

    opt.adainrefertype="img"
    opt.adaintraintype="twofake"
    opt.adain_layer_num=1
    opt.start_adain_layer=4
    opt.lambda_ADAIN=0.001
    opt.adainformin=False
    opt.lambda_mmaxL1=0.001
    ...
    opt.netD2 = "unetdiscriminator_1"
    opt.netG = "adainunetgenerator2"
        ...
        for i, data in enumerate(dataset):  # inner loop within one epoch
            ...
            with torch.no_grad():
                visualizer.showexpample((epoch-1)*dataset_size+(i*opt.batch_size), opt.display_epoch_pagenum, 1, to_img(model.real_B.cpu()),
                                      to_img(model.real_A.cpu()), to_img(model.fake_B).cpu(),to_img(model.maxalpha_fake).cpu(),
                                      num=4)
